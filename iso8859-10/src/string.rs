...
impl IsoLatin6String {
    ...
    pub fn from_iso8859_10(vec: Vec<u8>) -> Result<Self, FromIso8859_10Error> {
        todo!()
    }
...
// Non public API
impl IsoLatin6String {
    pub(crate) fn is_valid_iso8859_10(&self) -> bool {
        self.bytes
            .iter()
            .all(|&c| Self::byte_is_valid_iso8859_10(c))
    }

    fn byte_is_valid_iso8859_10(char: u8) -> bool {
        match char {
            0x00..=0x7E => true,
            0x7F..=0x9F => false,
            0xA0..=0xFF => true,
    ...
    fn map_byte_to_char(byte: u8) -> Option<char> {
        match byte {
            0x00..=0x7E => Some(byte as char),
            0x7F..=0x9F => None,
            c => match map::DECODE_MAP.get(c as usize - 0xA0) {
                Some(0) => None,
                Some(&c) => char::from_u32(c as u32),
                None => None,
            },
        }
    }
    ...
    /// Docs: TODO
    pub(crate) fn from_iso8859_10_unchecked(vec: Vec<u8>) -> Self {
        Self { bytes: vec }
    }
}
...
/// Tip: You can use the docs of `std::string::String` to get a better idea and inspiration
#[derive(Debug)]
pub struct FromIso8859_10Error {
    // TODO
}
